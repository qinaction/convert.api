...
//获取年龄、性别
func GetCitizenInfo(citizenNo []byte) (err error, age int, sex string) {
	...
	nLen := len(citizenNo)
	if nLen == 18 {
		citizenNo18 =citizenNo
	}else if nLen == 15 {
		citizenNo18 = Citizen15To18(citizenNo)
	}else {
		return
	}

	// Birthday information.
	nYear, _ := strconv.Atoi(string(citizenNo18[6:10]))
	//nMonth, _ := strconv.Atoi(string(citizenNo[10:12]))
	//nDay, _ := strconv.Atoi(string(citizenNo[12:14]))
	//fmt.Println(nYear,nMonth,nDay)
	if nYear <= 0 {
		age = 0
	}
	...
	age = nowyear - nYear
	// Gender information.
	genderMask, _ := strconv.Atoi(string(citizenNo18[16]))
	if genderMask % 2 == 0 {
		sex = "女"
	} else {
		sex = "男"
